...
class IEMOCAPDataset(Dataset):
    ...
    def __init__(self, path=None, train=True):
        ...
        self.len = len(self.keys) # len=120
    ...
    def __getitem__(self, index):
        ...
        return torch.FloatTensor(self.videoText[vid]), \
               ...
               vid
# videoText[vid]:54*100;videoVisual[vid]:54*512;videoAudio[vid]:54*100;54*2,1:男，0:女;videoLabels:54
...
class MELDDataset(Dataset):
    ...
    def __init__(self, path=None, n_classes=-1, train=True):
        ...
        '''
        label index mapping = {'neutral': 7, 'surprise': 1, 'fear': 2, 'sadness': 3, 'joy': 4, 'disgust': 5, 'anger':6}
        '''
        self.keys = [x for x in (self.trainVid if train else self.testVid)]
    ...
    def collate_fn(self, data):
        dat = pd.DataFrame(data)
        return [pad_sequence(dat[i],padding_value = -1) if i < 3 else pad_sequence(dat[i], True,padding_value=-1) if i < 5 else dat[i].tolist() for i in dat]
